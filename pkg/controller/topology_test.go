...
func TestTopologyAggregation(t *testing.T) {
	...
	for name, tc := range testcases {
		strictTopologyChecks := []bool{false, true}
		if !tc.hasSelectedNode {
			strictTopologyChecks = append(strictTopologyChecks, true)
		}
		for _, strictTopology := range strictTopologyChecks {
			if strictTopology {
				name += " with strict topology"
			}
			t.Logf("test: %s", name)
			...
			if requirements != nil && expectedRequisite == nil {
...
func TestPreferredTopologies(t *testing.T) {
	...
	for name, tc := range testcases {
		for _, strictTopology := range []bool{false, true} {
			...
			if requirements != nil && expectedPreferred == nil {
